...
class QuiqChatClient {
  ...
  private _ingestTranscriptItems = (
    ...
    const newTranscript = sortByTimestamp(unionBy(this.transcript, newItems, 'id'));
    const transcriptChanged = newTranscript.length > this.transcript.length;
    this.transcript = newTranscript;

    if (transcriptChanged && this.callbacks.onTranscriptChanged && sendNewMessageCallback) {
