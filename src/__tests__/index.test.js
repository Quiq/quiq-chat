...
describe("QuiqChatClient", () => {
  ...
  describe("start with an error", () => {
    beforeEach(() => {
      ...
      API.fetchWebsocketInfo.mockReturnValueOnce(
  ...
  describe("getting new messages", () => {
    ...
    it("calls onNewMessages", () => {
      expect(onNewMessages).lastCalledWith(
        initialConvo.messages.concat(newMessage)
      );
    });
  });
    ...
    describe("timeout logic when timer expires", () => {
      ...
      it("times out after appConstants.MINUTES_UNTIL_INACTIVE minutes", () => {
        ...
        expect(setTimeout.mock.calls[0][1]).toBe(
          MINUTES_UNTIL_INACTIVE * 60 * 1000 + 1000
        );
      });
    ...
    describe("ChatMessage", () => {
      describe("REGISTER", () => {
        ...
        it("sets meaningful action flag in local storage", () => {
          expect(storage.setQuiqUserTakenMeaningfulAction).toHaveBeenCalledWith(
            true
          );
        });
      });
