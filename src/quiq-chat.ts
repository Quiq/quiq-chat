      ...
      if (this.status < requiredStatus) {
        switch (requiredStatus) {
          ...
          case QuiqChatClientStatus.INITIALIZED:
            log.warn(
              `"${methodName}()" should not be called before initializing QuiqChat by calling the "initialize()" method.`,
              { capture: false },
            );
            break;
          case QuiqChatClientStatus.RUNNING:
            log.warn(
              `"${methodName}()" should not be called before starting QuiqChat by calling the "start()" method.`,
              { capture: false },
            );
            break;
          default:
...
class QuiqChatClient {
  ...
  async sendTextMessage(text: string) {
    ...
    return API.sendMessage({ text, context: ChatState.context });
  }

  @StatusRequired(QuiqChatClientStatus.RUNNING)
  async sendQuiqReply(reply: ReplyResponse) {
    await this._prepareForMessageSend();
    return API.sendMessage({ ...reply, context: ChatState.context });
  }
  ...
  async sendAttachmentMessage(
    ...
    const { id } = await API.sendAttachmentMessage(uploadDirective.uploadId, ChatState.context);
